  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 text-slate-200 p-6 overflow-hidden">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 shrink-0">
            ...
            <button onClick={() => { setEditingTemplate(null); setIsModalOpen(true); }} className="flex items-center gap-2 bg-cyan-600 hover:bg-cyan-500 text-white px-4 py-2 rounded-lg transition-all shadow-lg shadow-cyan-900/20 font-bold text-xs uppercase tracking-widest">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4 animate-in fade-in duration-300">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden border-t-4 border-t-cyan-600 flex flex-col max-h-[90vh]">
        ...
        <div className="p-5 bg-slate-950/80 border-t border-slate-800 flex justify-end gap-3">
             ...
             <button onClick={(e) => { e.preventDefault(); onSave({ ...formData, updatedAt: Date.now() }); onClose(); }} className="px-6 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-black uppercase tracking-widest rounded-lg shadow-lg shadow-cyan-900/20 transition-all flex items-center gap-2"><Save size={16} /> Deploy Blueprint</button>
        </div>
      </div>
    </div>
  );
